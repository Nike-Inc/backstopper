# Backstopper Sample Application - spring-boot1
...
Servlet filter before the request ever hits Spring. The `BackstopperSpringboot1ContainerErrorController`
from the `backstopper-spring-boot1` dependency registers itself to handle these non-Spring container errors, where it 
is passed off to Backstopper.
