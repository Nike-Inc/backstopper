...
public abstract class ReflectionBasedJsr303AnnotationTrollerBase {
     ...
     * called.
     */
    private Set<String> DEFAULT_CONSTRAINT_SEARCH_PACKAGES = new LinkedHashSet<>(Arrays.asList(
        "com.nike",
    ...
    private List<Pair<Annotation, AnnotatedElement>> setupAllConstraintAnnotationsMasterList(
        ...
        for (Class<? extends Annotation> constraintAnnotationClass : constraintAnnotationClassesArg) {
            ...
            List<Class<?>> typesAnnotatedWithThisClass = new ArrayList<>();
            ...
            for (Class<?> annotatedClassType : typesAnnotatedWithThisClass) {
                ...
                if (!annotatedClassType.isAnnotation()) {
                    ...
                    for (Annotation annotationToRegister : annotationsToRegister) {
                        masterList.add(Pair.<Annotation, AnnotatedElement>of(annotationToRegister, annotatedClassType));
                    }
                }
            }

            // Grab the method params annotated with this class.
            List<Method> methodParamsAnnotatedWithThisClass = new ArrayList<>();
            methodParamsAnnotatedWithThisClass
                .addAll(reflectionsArg.getMethodsWithAnyParamAnnotated(constraintAnnotationClass));
            ...
            List<Constructor> constructorParamsAnnotatedWithThisClass = new ArrayList<>();
            constructorParamsAnnotatedWithThisClass
                .addAll(reflectionsArg.getConstructorsWithAnyParamAnnotated(constraintAnnotationClass));
    ...
    public static List<Pair<Annotation, AnnotatedElement>> getSubAnnotationListForElementsOfOwnerClass(
        List<Pair<Annotation, AnnotatedElement>> listToFilter, final Class ownerClass) {
        return getSubAnnotationList(listToFilter, new Predicate<Pair<Annotation, AnnotatedElement>>() {
            public boolean apply(Pair<Annotation, AnnotatedElement> input) {
                return getOwnerClass(input.getRight()).equals(ownerClass);
            }
        });
    }
    ...
    public static List<Pair<Annotation, AnnotatedElement>> getSubAnnotationListForAnnotationsOfClassType(
        ...
        return getSubAnnotationList(listToFilter, new Predicate<Pair<Annotation, AnnotatedElement>>() {
            public boolean apply(Pair<Annotation, AnnotatedElement> input) {
                return input.getLeft().annotationType().equals(desiredAnnotationClass);
            }
        });
    }
